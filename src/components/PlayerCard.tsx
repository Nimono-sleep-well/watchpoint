...
export function PlayerCard({ id, handle, realName, country, role, currentTeam }: PlayerCardProps) {
  return (
    <Link
      href={`/players/${id}`}
      className="block rounded-lg border border-[#2a2d3a] bg-[#1a1d27] p-4 hover:border-[#f99e1a] transition-colors"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="truncate font-bold text-[#e5e7eb] text-base">{handle}</span>
          </div>
          {realName && (
            <p className="mt-0.5 truncate text-sm text-[#6b7280]">{realName}</p>
          )}
        </div>
        {role && (
          <span
